//
// Advent of Code 2020, day 20, part two
//
...
bool tag_sea_monster(std::vector<std::string> &image, int x, int y)
{
	...
	if (y + monster.size() > image.size()
	 || x + monster[0].size() > image[0].size()) {
		return false;
	}

	for (int my = 0; my < monster.size(); ++my) {
		for (int mx = 0; mx < monster[my].size(); ++mx) {
			if (monster[my][mx] == '#') {
				if (image[y + my][x + mx] == '.') {
					return false;
				}
...
std::size_t tag_sea_monsters(std::vector<std::string> &image)
{
	std::size_t num = 0;

	for (int y = 0; y < image.size(); ++y) {
		for (int x = 0; x < image[y].size(); ++x) {
			num += static_cast<std::size_t>(tag_sea_monster(image, x, y));
		}
	}
...
void tag_all_sea_monsters(std::vector<std::string> &image)
{
	for (auto op : "vhvrvhv") {
		if (tag_sea_monsters(image) > 0) {
			return;
		}
...
int main()
{
	...
	for (int tile_y = y_max; tile_y >= y_min; --tile_y) {
		std::vector<std::string> rows(tiles[map[{x_min, y_min}]].data.size() - 2);

		for (int tile_x = x_min; tile_x <= x_max; ++tile_x) {
			const auto &tile = tiles[map[{tile_x, tile_y}]];

			for (int y = 1; y < tile.data.size() - 1; ++y) {
				rows[y - 1].append(tile.data[y].substr(1, tile.data[y].size() - 2));
			}
		}

		image.insert(image.end(), rows.begin(), rows.end());
	}

	tag_all_sea_monsters(image);
